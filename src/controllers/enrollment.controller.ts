  ...
  try {
    ...
    if (search) {
      enrollments = enrollments.filter((enrollment: any) =>
        enrollment.student?.name?.toLowerCase().includes((search as string).toLowerCase()) ||
        enrollment.student?.email?.toLowerCase().includes((search as string).toLowerCase())
      );
    }
...
export const getEnrollmentsByBatch = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const getEnrollmentStats = async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
